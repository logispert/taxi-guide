...
1. 고객이 택시등급을 선택하고 택시를 호출 한다.(택시호출서비스)
1. 고객은 요금을 결제한다.(요금결재관리)
1. 고객 위치에서 가입된 가까운 택시에게 고객위치와 고객 목적지를 전달한다.(택시호출서비스 -> 택시관리) 고객과 택시의 가까운 순을 위해서 거리를 속성으로...
1. 택시는 호출요청을 확인하고 고객 위치로 출발한다.(택시기사 -> 택시관리)
1. 호출요청 확인 수신 즉시, 고객에게 할당된 택시정보를 전송한다.(택시관리 -> 택시호출서비스)
1. 호출요청을 확인하는 택시는 호출 서비스 가입자이다(택시관리)
1. 택시기사는 호출요청을 무시할 수 있다.(택시기사 -> 택시관리)
1. 고객은 택시호출을 취소 할 수 있다.(택시호출서비스)
1. 호출이 취소 되면 택시기사에게 취소알림을 보낸다.(택시관리)
1. 고객은 택시기사 위치를 중간중간 조회 한다.(고객)

비기능적 요구사항
1. 트랜잭션
    1. 결제가 완료되지 않으면 택시기사를 섭외할 수 없다.  Sync 호출 
1. 장애격리
    1. 택시 예약은 택시기사 관리 기능이 동작하지 않더라도, 365일 24시간 받을 수 있어야 한다 Async (event-driven), Eventual  Consistency
    1. 결제시스템이 과중되면 사용자를 잠시동안 받지 않고 결제를 잠시후에 하도록 유도한다 Circuit breaker, fallback
1. 성능
    1. 택시 기사가 할당 된 고객의 경우 택시기사의 위치를 확인 할 수 있어야 한다.  Event driven
